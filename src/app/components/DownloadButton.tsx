...
const stores: {
  ...
  "Google Play": {
    ...
    link: "https://play.google.com/store?hl=en_US",
  },
};
...
export default function DownloadButton({
  ...
  return (
    <a target="_blank" href={stores[store].link} rel="noopener noreferrer">
      <button
        className={
          `gap-4 flex items-center w-full justify-center md:w-auto py-2 px-6 border-2 border-current max-h-18` +
      ...
      >
        {
          <svg
            ...
            height="2em"
            viewBox={stores[store].viewBox}
            fill={hover ? "black" : "white"}
          >
            <path d={stores[store].pathD} />
          </svg>
        }
        <div className="flex flex-col text-left">
          <span className={!hover ? "text-gray-400" : "text-gray-800"}>
            Download on the
          </span>
